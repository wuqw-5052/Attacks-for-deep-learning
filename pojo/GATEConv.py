...
class Edge_GATConv(MessagePassing):
    def __init__(self,
        ...
        self.edge_update_here = Parameter(torch.Tensor(out_channels + edge_dim, out_channels))   # [emb(out)+edge_dim] x emb(out)  # new
    ...
    def reset_parameters(self):
        glorot(self.weight)
        glorot(self.att)
        glorot(self.edge_update_here)  # new
        zeros(self.bias)
    ...
    def update(self, aggr_out):   # 4. Return node embeddings (average heads)
        ...
        # print('self.edge_update_here', self.edge_update_here)   # (emb(out)+edge_dim) x emb(out)
        aggr_out = torch.mm(aggr_out, self.edge_update_here)
